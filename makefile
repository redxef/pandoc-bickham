...
all: $(SOURCE_DOCS_TEX) $(SOURCE_DOCS_MD)
	docker run --mount src=$(shell pwd),target=/mnt,type=bind pandoc-bickham /bin/sh -c 'cd /mnt && make --file /docker-makefile'

clean:
	docker run --mount src=$(shell pwd),target=/mnt,type=bind pandoc-bickham /bin/sh -c 'cd /mnt && make --file /docker-makefile clean'
